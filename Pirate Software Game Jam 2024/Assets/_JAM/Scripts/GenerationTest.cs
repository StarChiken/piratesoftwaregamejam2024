...
namespace Base.Gameplay
{
    public class GenerationTest : MyMonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                Collider2D col = Physics2D.OverlapCircle(Camera.main.ScreenToWorldPoint(Mouse.current.position.value),
                    0.25f);
        ...
        private void GenerateGrid(int houses)
        {
            ...
            for (int x = 0; x < gridX; x++)
            {
                for (int y = 0; y < gridZ; y++)
                {
                    ...
                    GameObject buildingObject = Instantiate(buildings[randomBuildingIndex], position,
                        Quaternion.Euler(0, 0, Random.Range(1, 4) * 90));
                    ...
                    Building building = new Building($"Grid {x} {y}", buildingChildrenPositions,
                        (BuildingSize)randomBuildingIndex, buildingObject, (BuildingType)buildingColorIndex);

                    SpriteRenderer[] spriteRenderers =
                        building.buildingObject.GetComponentsInChildren<SpriteRenderer>();
